...
int main(int argc, char* argv[]) {
    ...
    auto* sendSink = new FileSinker("./test.png");
    ...
    QObject::connect(&clientPeer, &PeerConnection::connected, [&]() {
        ...
        senderController.sendData(1024); // 按 8 字节分块发送
    });

    // -------- 8) 收尾检查 --------
    QTimer::singleShot(2000, [&]() {
        auto* memRecv = dynamic_cast<ImageSinker*>(recvSink);
        if (memRecv) {
            qDebug() << "[final] received size =" << memRecv->size();
            QImage image = memRecv->getImage();
            if (image.isNull()) {
                qDebug() << "Image null!";
            } else {
                image.save("other_test.png");
            }
        }
        app.quit();
    });
